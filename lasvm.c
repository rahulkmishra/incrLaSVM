...
void initialize_lasvm_load(lasvm_t *self)
{
	int nl = self->maxl;
	self->alpha = (real_t*)xrealloc(self->alpha, nl*sizeof(real_t));
	self->cmin = (real_t*)xrealloc(self->cmin, nl*sizeof(real_t));
	self->cmax = (real_t*)xrealloc(self->cmax, nl*sizeof(real_t));
	self->g = (real_t*)xrealloc(self->g, (nl)*sizeof(real_t));
}
