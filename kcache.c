...
xextend(lasvm_kcache_t *self, int k, int nlen)
{
	...
	if (nlen > olen)
	{
		...
		self->rdata[k] = ndata;
		self->rsize[k] = nlen;
		self->cursize += (long long)(nlen - olen) * sizeof(float);
	}
}
...
void 
lasvm_kcache_set_maximum_size(lasvm_kcache_t *self, long long entries)
{
	ASSERT(self);
	ASSERT(entries>0);
...
void initialize_kcache_load(lasvm_kcache_t *self)
{
		int nl = self->l;
		self->i2r = (int*)xrealloc(self->i2r, nl*sizeof(int));
		self->r2i = (int*)xrealloc(self->r2i, nl*sizeof(int));
		self->rsize = (int*)xrealloc(self->rsize, nl*sizeof(int));
		self->qnext = (int*)xrealloc(self->qnext, (nl)*sizeof(int));
		self->qprev = (int*)xrealloc(self->qprev, (nl)*sizeof(int));
		self->rdiag = (float*)xrealloc(self->rdiag, nl*sizeof(float));
		self->rdata = (float**)xrealloc(self->rdata, nl*sizeof(float*));
}
